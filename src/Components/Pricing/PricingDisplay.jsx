...
const PricingDisplay = ({ price }) => {
  ...
  return (
    <div className="mt-7 mb-7 relative">
      {
        price.highlighted? <p className="absolute -top-3 left-40   badge badge-warning">{price.tag}</p>: ''
      }
        ...
        <p className={`text-[24px] font-bold ${price.highlighted? "text-white" :"text-[#101727]"} `}>{price.name}</p>
        <p className={` ${price.highlighted? "text-white" :"text-[#627382]"}`}>{price.tagline}</p>
        <p className=""><span className={`text-[30px] font-bold ${price.highlighted? "text-white" :"text-[#101727]"}`}>${price.monthlyPrice}</span><span className={` ${price.highlighted? "text-white" :"text-[#627382]"}`}>/{price.period}</span></p>
       {
