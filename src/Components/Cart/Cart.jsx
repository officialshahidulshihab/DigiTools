    ...
    <div className="w-11/12 mx-auto mt-20 mb-20">
      ...
      <div className="card bg-base-100   shadow-sm">
        <div className="card-body">
          ...
          </div> : <div> {cartData.map((prod, i)=><CartDisplay handleRemove={handleRemove} prod={prod} key={i}></CartDisplay>)}
          {<p className="text-lg font-semibold">Total :                                             $ {  cartData.reduce((total, item) =>total + item.price, 0)  }</p>
          } 
          {<button onClick={()=>handleCheckout()} className="w-full btn bg-linear-to-r from-[#4f39f6] to-[#9514fa]  rounded-2xl text-white">Proceed to Checkout</button>}</div>
